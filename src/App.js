      ...
      <div className="m-6 p-3 mt-10 ml-0 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-6 xl:gap-x-10" style={{ maxHeight: '800px', overflowY: 'scroll' }}>
        ...
        {ProductsCategory.map((product, index) => (
          <div key={index} className="group relative shadow-lg" >
            <div className=" min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-150 lg:aspect-none">
              <img
                alt="Product Image"
                src={product.image}
              ...
              <span style={{ width: '500px' }}>{product.image}</span>
            </div>
            <div className="flex justify-between p-3">
              ...
              <p className="text-sm font-medium text-green-600">${product.description}</p>
            </div>
          </div>
        ))}
      </div>
